...
BoardDTO fdto = fdao.getBoard(2,idx);
%>
<!-- 게시판 -->
<article>
<h1>Notice</h1>
<form action="fUpdatePro.jsp" method="post" name="fr" enctype="multipart/form-data">
<input type="hidden" name="modify_id" value=<%=session.getAttribute("id")%>>
<input type="hidden" name="id" value=<%=session.getAttribute("id")%>>
<input type="hidden" name="pageNum" value=<%=pageNum%>>
<input type="hidden" name="idx" value=<%=fdto.getIdx()%>>
	<div class="write_subject">
		<p><input type="text" name="subject" class="subject" placeholder="제목을 입력해주세요." value="<%=fdto.getSubject() %>"></P>
		</div>	
		<div class="write_form">
			<TEXTAREA id=editor style="DISPLAY: none; VISIBILITY: hidden" name=content><%=fdto.getContent() %></TEXTAREA>
			<script>
