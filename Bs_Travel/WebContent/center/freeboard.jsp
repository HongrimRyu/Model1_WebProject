...
<html>
	...
	<body>
		...
		int maxNum=0;
		if(search.trim().equals("")) maxNum=bdao.maxIdx(2);//총글갯수
		else maxNum=bdao.maxIdx(2,search);//총글갯수
		int pageSize=10;//한화면에 출력되는 게시글 갯수;
		int currentPage=Integer.parseInt(pageNum);
		int startRow=(currentPage-1)*pageSize + 1;
		ArrayList<BoardDTO> al = null;
		if(search.trim().equals("")) al=(ArrayList<BoardDTO>)bdao.getBoardList(2,startRow,pageSize);
		else al=(ArrayList<BoardDTO>)bdao.getBoardList(2,startRow,pageSize,search);
		%>
		<div id="wrap">
		<!-- 헤더들어가는 곳 -->
		...
		<article>
			...
			<table id="notice">
				...
				<tr><td><%=bdto.getIdx() %></td>
					...
					<%if(bdto.isDel_state()){ %>
					<%if(bdto.getRe_lev()!=0){ %>
					<%for(int i=0 ; i<bdto.getRe_lev();i++){ %>
					&nbsp;
					<%} %>
					[답글]
					<%} %>
					<a href="fContent.jsp?idx=<%=bdto.getIdx()%>&pageNum=<%=currentPage%>">
			...
			<div id="table_search">
				<form action="notice.jsp" name="search" method="post">
					...
					<a href="fWrite.jsp?pageNum=<%=pageNum%>"><input type="button" value="write" class="btn" ></a>
				</form>
			</div>
			<div class="clear"></div>
			<div id="page_control">
