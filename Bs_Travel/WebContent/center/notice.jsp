...
int maxNum=0;
if(search.trim().equals("")) maxNum=bdao.maxIdx(1);//총글갯수
else maxNum=bdao.maxIdx(1,search);//총글갯수
int pageSize=10;//한화면에 출력되는 게시글 갯수;
int currentPage=Integer.parseInt(pageNum);
int startRow=(currentPage-1)*pageSize + 1;
ArrayList<BoardDTO> al = null;
if(search.trim().equals("")) al=(ArrayList<BoardDTO>)bdao.getBoardList(1,startRow,pageSize);
else al=(ArrayList<BoardDTO>)bdao.getBoardList(1,startRow,pageSize,search);
%>
<div id="wrap">
<!-- 헤더들어가는 곳 -->
...
<tr><td><%=bdto.getIdx() %></td>
	<td class="left">
	<%if(bdto.isDel_state()){ %>
	<a href="nContent.jsp?idx=<%=bdto.getIdx()%>&pageNum=<%=currentPage%>">
	<%}%>
...
if(id.equals("hongadmin")){ %>
<a href="nWrite.jsp?pageNum=<%=pageNum%>"><input type="button" value="글쓰기" class="btn" ></a>
<%}else{%>
